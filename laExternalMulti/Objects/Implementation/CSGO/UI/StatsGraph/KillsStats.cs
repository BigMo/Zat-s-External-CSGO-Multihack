...
{
    class KillsStats : StatsGraph
    {
        #region CONSTRUCTOR
        public KillsStats(Theme theme, TextFormat font, float x, float y, float width, float height)
            : base(theme, font, x, y, width, height, "Kills") { }
        #endregion
        protected override void DrawPlayer(SharpDX.Direct2D1.WindowRenderTarget device, Player player, int index, int numPlayers)
        {
            float
                ...
                rectHeight = this.Height / MaxValue * player.Kills,
            ...
            DrawText(device,
                ...
                Theme.ShadowOffsetX,
                Theme.ShadowOffsetY,
                String.Format("{0} ({1})", player.Name, player.Kills),
                this.Font);
        }
